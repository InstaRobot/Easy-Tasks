...
extension MainViewController {
    ...
    private func fetchLessons() {
        ...
        let preparedLessons: [LessonModel] = objects.map {
            return LessonModel(id: $0.id, name: $0.name, color: $0.color, tasks: $0.tasks.map { task in
...
extension MainViewController {
    ...
    private func setData() {
        guard
            ...
            self.tasksForTodayLabel.text = "Нет задач"
            return
        }
        ...
        self.tasksForTodayLabel.text = "Всего задач: \(allTasksCount) / Выполнено: \(doneCount)"
    }
